(function () {
    ...
    //*** Preload de toutes les images appelées dans le canvas avant le lancement du jeu
    var imgLoaded = 0;
    var imgCollection= [];
    ...
    // * Vérifie que toutes les images soient chargées avant de pouvoir lancer le jeu
    function checkLoad (){
    ...
    // * Musique non obligatoire pour le lancement du jeu
    ...
    function loader (){
        ...
        var p1 = new Pateforme (0,100,can.height - (param.plateforme.hauteur / 2), 20);
...
//*** Constructeur des principaux objets du jeux : les plateformes
    function Pateforme (plateformeSelected,x,y,nbBriqueCentral){
        ...
        //* Calcule de la largeur : addition de toutes les largeur d'images
            ...
            var XPos = (i*this.imgCentre[centreSelected].width) + this.imgDebut.width; // * Position
                ...
                if (centreImage.src.substring(centreImage.src.length-16) === "ralentisseur.png"){
                    if (this.ralentisseur) {
                        this.ralentisseur[1] = XPos + centreImage.width;
                    }
                    else {
        ...
        this.isFond = function (){
            ...
            if (this.x < perso.x + perso.spriteSize && this.x + this.largeur > perso.x){
                ...
                if (this.ralentisseur && perso.toucherLeFond){
                    if (this.ralentisseur[0]+ this.x < perso.x && this.ralentisseur[1] + this.x > perso.x){
...
// *** objet générateur de particules lumineuses représentant le but à atteindre
    var light = {
        ...
        maj: function(){
            if (nbLight === 5) this.redLight(); // * Objectif caché
            this.anim();//* animation
            this.glisse(); //* position
...
// *** Fonctions de mise à jours générales du jeu ***
    ...
    function timer  (){ // * Pas de timestamp : maj à chaque raffraichissement de l'écran
        fresh();
